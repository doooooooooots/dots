...
const InputNumber = (props) => {
  ...
  const handleChange = (e) => {
    if (e.target.name === 'userMaxCol') {
      store.setUserData('userMaxCol', Math.min(Math.max(1, e.target.value), store.maxCol));
      return true;
    }

    if (e.target.name === 'userMaxRow') {
      store.setUserData('userMaxRow', Math.min(Math.max(1, e.target.value), store.maxRow));
      return true;
    }
  ...
  return (
    <Input
      ...
      disabled={
        !!(
          (element.id === 'PigeMX1' && store.getUserDatas('isPigeSymetrical')) ||
