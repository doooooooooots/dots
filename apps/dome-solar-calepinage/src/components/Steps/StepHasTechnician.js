      ...
      <DrawerContentSource>
        <StepDrawerContainer onNext={handleNext}>
          {isEmpty(actionsHistory) && !isEmpty(store.currentDefaultTarget) && (
            <Fields typeOf='cladding' values={store.currentDefaultTarget} defaultFields={['name']} mode='view' />
          )}
        </StepDrawerContainer>
      </DrawerContentSource>
        ...
        {isEmpty(actionsHistory) && (
          <Box textAlign='center' my={3}>
            {store.steps.indexOf(store.form.currentStep) !== 0 && isEmpty(formsHistory) ? (
              <Button variant='outlined' onClick={store.previous} startIcon={<NavigateBeforeIcon />} sx={{ mr: 1 }}>
                Retour
              </Button>
            ) : (
              <Box />
