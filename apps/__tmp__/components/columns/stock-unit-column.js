...
import formatCurrency from '@helpers/currency/format-currency';
import FieldConditionShow from '@components/field-condition-show';
import FieldCountryFlagShow from '@components/field-country-flag-show';
import FieldIsFirstEdShow from '@components/field-is-first-ed-show';
import ButtonOpenDetails from '@components/btn-open-details';
...
export const STOCK_UNIT_COLUMNS = {
  from: {
    field: 'from',
    headerName: 'from',
    renderCell: ({ row }) => <FieldCountryFlagShow countryCode={row.seller.address.country} />,
    width: 40
  },
  productId: {
    field: 'productId'
  ...
  expansion: {
    field: 'expansion',
    valueGetter: ({ row: { expansion } }) => (isObject(expansion) ? expansion.abbreviation : expansion)
  },
  expAbbreviation: {
  ...
  image: {
    ...
    renderCell: (params) => (
      <Tooltip
        title={<Image src={`${params.value}`} alt='preview' width={251} height={364} unoptimized />}
        placement='right-end'
        followCursor
      >
        <Box
...
export const actions = {
  actionAddOffer: {
    getActions: () => [
      <GridActionsCellItem key='create' label='Create' icon={<EditOutlinedIcon />} />,
      <GridActionsCellItem key='print' label='Print' icon={<EditOutlinedIcon />} showInMenu />
    ]
  }
};
