...
const options = ['yugioh', 'pokemon'];

export default function GameSplitButton(props) {
  ...
  const handleClick = () => {
    onClick(options[selectedIndex] === 'pokemon' ? 6 : 3);
  };
  ...
  return (
    <React.Fragment>
      <ButtonGroup variant='contained' ref={anchorRef} aria-label='split button' {...other}>
              ...
              <ClickAwayListener onClickAway={handleClose}>
                <MenuList id='split-button-menu'>
                  {options.map((option, index) => (
                    ...
                    >
                      <Image
                        src={option === 'pokemon' ? pokemonLogo : ygoLogo}
                        alt='Game logo'
                        width={110}
                        height={30}
                        unoptimized
