...
import addOAuthInterceptor from './axios-middleware-o-auth-interceptor';
...
axiosInstance.interceptors.request.use((request) => {
  ...
  headers.authorization = (authorization as string).replace(strToReplace, encodeURIComponent(url.split('?')[0]));
  return request;
});
